...
, HIRE_DATE, SALARY FROM EMPLOYEES WHERE HIRE_DATE LIKE '03%';
--10. 모든 사원의 이름과 급SELECT FIRST_NAME|| ' ' || LAST_NAME AS NAME여를 급여가 많은 사원부터 적은 사원순서로 출력하세요.
SELECT FIRST_NAME|| ' ' || LAST_NAME AS NAME, SALARY FROM EMPLOYEES ORDER BY SALARY;
...
SELECT FIRST_NAME||' '||LAST_NAME AS NAME, JOB_ID FROM EMPLOYEES WHERE JOB_ID IN ('IT_PROG','SA_MAN');
