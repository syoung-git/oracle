...
GRANT CREATE SESSION, CREATE TABLE, CREATE SEQUENCE, CREATE VIEW, CREATE PROCEDURE TO USER01; --USER01한테 권한부여
--테이블 스페이스 (데이터를 저장하는 물리적인 공간) 지정
ALTER USER USER01 DEFAULT TABLESPACE USERS QUOTA UNLIMITED ON USERS;
--수정한다 유저 USER01 기본 테이블스페이스는 USERS이고, 무제한으로 사용한다 ON USERS테이블 스페이스를
...
GRANT CONNECT, RESOURCE TO USER01;--CONNECT:접속 ROLE, RESOURCE:개발 ROLE, DBA: 관리자 ROLE
