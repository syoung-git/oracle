--문자열 함수
SELECT LOWER('H') FROM DUAL; --SQUL을 간단히 연습하기 위한 가상 테이블
...
SELECT FIRST_NAME, INSTR(FIRST_NAME, 'a') FROM EMPLOYEES; --a가 있는 위치 반환/ 없으면 0반환
--SUBSTR 문자열 자르기
SELECT FIRST_NAME, SUBSTR(FIRST_NAME, 3), SUBSTR(FIRST_NAME, 3, 2) FROM EMPLOYEES; --3미만 절삭, 3번째 위치에서 2개 자름
 ...
 SELECT LPAD('ABC', 10, '*') FROM DUAL;--ABC를 10칸 잡고, 나머지 부분 왼쪽부터 *채움
...
SELECT CONCAT(FIRST_NAME, LAST_NAME), REPLACE(HIRE_DATE,'/','') FROM EMPLOYEES ORDER BY FIRST_NAME ASC;
